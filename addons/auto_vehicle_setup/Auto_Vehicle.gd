...
@export_category("RigidBody Overrides")
## The total weight of the vehicle in kilograms
@export var vehicle_mass: float = 1500.0:
	set(value):
		vehicle_mass = value
		mass = value

## Lowers the center of mass on the Y-axis to prevent tipping (-0.6 is ideal)
@export var center_of_mass_y_offset: float = -0.6:
	set(value):
		center_of_mass_y_offset = value
...
@export var target_length: float = 0.35        
...
func _ready() -> void:
	# Force apply the inspector variables to the underlying RigidBody3D properties
	mass = vehicle_mass
	_update_center_of_mass()
...
func _update_center_of_mass() -> void:
	# Keep it automatic so your original tuning stays intact
	center_of_mass_mode = RigidBody3D.CENTER_OF_MASS_MODE_AUTO
	# Removing the manual center_of_mass assignment stops the engine error
